...
var fncLoginSubmit = function(){
	if($("#mbCellNo1").val() == "" || $("#mbCellNo1").val().replace(/(^\s+)|(\s+$)/g, "") == ""){
		alert("핸드폰번호 1번째 자리를 입력하여 주시기 바랍니다.");
		$("#mbCellNo1").focus();
		return false;
	}

	if($("#mbCellNo2").val() == "" || $("#mbCellNo2").val().replace(/(^\s+)|(\s+$)/g, "") == ""){
		alert("핸드폰번호 2번째 자리를 입력하여 주시기 바랍니다.");
		$("#mbCellNo2").focus();
		return false;
	}

	if($("#mbCellNo3").val() == "" || $("#mbCellNo3").val().replace(/(^\s+)|(\s+$)/g, "") == ""){
		alert("핸드폰번호 3번째 자리를 입력하여 주시기 바랍니다.");
		$("#mbCellNo3").focus();
		return false;
	}
	...
	$.post("/rebates/loginProcess.json", {"mbCellNo":$("#mbCellNo1").val()+$("#mbCellNo2").val()+$("#mbCellNo3").val(), "mbPwd":$("#mbPwd").val()}, function (result) {
		if(typeof(result.resultCode) == "number" && result.resultCode == 1){
			if($("#returnUrl").val() != ""){
				location.href=decodeURIComponent($("#returnUrl").val().replace(/\+/g, " "));
			}else{
				location.href="/rebates/apply/main.do";
			}
		}else if(typeof(result.resultCode) == "number" && result.resultCode == 2){
			location.href="/rebates/pwdModify.do";
...
var fncAuthSubmit = function(){
	...
	$.post("/rebates/joinDupl.json", {"mbCellNo":$("#mbCellNo1").val()+$("#mbCellNo2").val()+$("#mbCellNo3").val(), "mbName": $("#mbName").val()}, function (result) {
		if(typeof(result.resultCode) == "number" && result.resultCode != 1){
			$("#duplInfo").show();	// 이미 회원가입이 되어있는 휴대폰 번호입니다. 로그인 또는 비밀번호 찾기를 통해 접속해 주시기 바랍니다.
		}else{
			$("#defaultFrame").attr("src", "/auth/front/checkPlus/checkPlusMain.do?type=join");
		}
	}, "json");
	return false;
};
...
var isAuthSubmit = false;

var fncAuthCheckSubmit = function(){
	if(isAuthSubmit == true){
		alert("회원 가입 처리중입니다. 잠시 기다려주시기 바랍니다.");
		return false;
	}
	$.post("/rebates/joinProcess.json", {"mbCellNo":$("#mbCellNo1").val()+$("#mbCellNo2").val()+$("#mbCellNo3").val(), "mbName":$("#mbName").val(), "mbPwd":$("#mbPwd").val(), "reMbPwd":$("#reMbPwd").val()}, function (result) {
		isAuthSubmit = true;
		if(typeof(result.resultCode) == "number" && result.resultCode == 1){
...
var fncFindCheckSubmit = function(){
	...
	$.post("/rebates/findPwdCheck.json", {"mbCellNo":$("#mbCellNo1").val()+$("#mbCellNo2").val()+$("#mbCellNo3").val(), "mbName" : $("#mbName").val()}, function (result) {
		if(typeof(result.resultCode) == "number" && result.resultCode == 0){
			alert("입력하신 정보와 일치하는 회원 정보가 없습니다.");
		}else{
			$("#defaultFrame").attr("src", "/auth/front/checkPlus/checkPlusMain.do?type=findPwd");
		}
	}, "json");
...
var fncFindActionSubmit = function(){
	$.post("/rebates/findPwdAction.json", {"mbCellNo":$("#mbCellNo1").val()+$("#mbCellNo2").val()+$("#mbCellNo3").val(), "mbName" : $("#mbName").val()}, function (result) {
		if(typeof(result.resultCode) == "number" && result.resultCode == 0){
			alert("입력하신 정보와 일치하는 회원 정보가 없습니다.");
		}else{
			alert("고객님의 휴대폰으로 초기화된 비밀번호가 전송되었습니다. 로그인 화면에서 핸드폰 번호와 초기화된 비밀번호를 입력해서 로그인하신 후에 비밀번호를 변경하시기 바랍니다.");
			location.href="/rebates/login.do";
		}
	}, "json");
	return false;
};
...
var fncPwdModifySubmit = function(){
	...
	$.post("/rebates/modifyPwd.json", {"orgMbPwd":$("#orgMbPwd").val(), "mbPwd":$("#mbPwd").val(), "reMbPwd":$("#reMbPwd").val()}, function (result) {
		if(typeof(result.resultCode) == "number" && result.resultCode == 1){
...
var fncAccountModify = function(seqNo){
	$("#seqNo").val(seqNo);
	$("#defaultFrm").attr({"action": "/rebates/apply/accountModify.do", "method" : "post", "target" : "_self"}).submit();
};

var fncCancelSubmit = function(){
	if(confirm("환급신청을 취소하시겠습니까?\n취소 하시더라도 다시 환급이 신청 가능합니다.")){
		$("#defaultFrm").attr({"action": "/rebates/apply/cancelAction.do", "method" : "post", "target" : "_self"}).submit();	
	}
};
...
var fncTempSubmit = function(){
	...
	if($("#isBranchInfo").val() != "Y"){
		alert("조회버튼을 통해 구매매장 정보를 입력하셔야 합니다.");
		return false;
	}
	
	if($("#productCd").val() == "" || $("#productCd").val().replace(/(^\s+)|(\s+$)/g, "") == ""){
		alert("제품검색 버튼을 통해 제품정보를 입력하셔야 합니다.");
		$("#btn_search").focus();
		return false;
	}
...
var fncApplySubmit = function(){
	...
	if($("#buyDivnCd").val() == "ON" && $("#buyNm").val() != "ETC" && $.trim($("#orderNo").val()) == ""){
...
*/
	...
	$("#salePrice").val(uncomma($("#salePrice").val()));
	$.post("/rebates/apply/getRefundPrice.json?seqNo="+$("#seqNo").val()+"&category="+$("#category").val()+"&productCd="+$("#productCd").val()+"&salePrice="+uncomma($("#salePrice").val()), function (result) {
		if(typeof(result.resultCode) == "number" && result.resultCode == "1"){
...
var fncGetModelDupl = function(){
	$.post("/rebates/apply/getModelDupl.json", {"productCd":$("#productCd").val()}, function (result) {
		if(typeof(result.resultCode) == "number" && result.resultCode == "1"){
			return true;
		}else{
			alert(result.message);
...
var fncRefundPrice = function(){
	...
	$.post("/rebates/apply/getRefundPrice.json", {"seqNo":$("#seqNo").val(), "salePrice":uncomma($("#salePrice").val())}, function (result) {
...
var fncSerialDupl = function(){
	...
	$.post("/rebates/apply/getSerialDupl.json", {"serialNo":$("#serialNo").val(), "category":$("#category").val(), "productCorpNm":$("#productCorpNm").val(), "seqNo":$("#seqNo").val()}, function (result) {
...
var fncAccountAuth = function(){
	...
	if($("#accountNo").val() == ""){
		...
		$.post("/rebates/apply/getAccountAuth.json", {"accountNm":$("#accountNm").val(), "bankCd":$("#bankCd").val(), "accountNo":$("#accountNo").val()}, function (result) {
			...
			if(typeof(result.resultCode) == "number" && result.resultCode == "1"){
				...
				if(result.resultCode == "B1999") $("#authAccountMsgFail").html("정상적인 은행계좌가 아닙니다. 확인 바랍니다.(예, 휴면계좌 등)");
				else if(result.resultCode == "D104") $("#authAccountMsgFail").html("해당 은행의 전산 장애로 계좌인증 서비스가 불가합니다.");
				else if(result.resultCode == "D105") $("#authAccountMsgFail").html("해당 은행의 전산 장애로 계좌인증 서비스가 불가합니다.");
				else if(result.resultCode == "TIME") $("#authAccountMsgFail").html("해당 은행의 전산 장애로 계좌인증 서비스가 불가합니다.");
				else if(result.resultCode == "DSYS") $("#authAccountMsgFail").html("해당 은행의 전산 장애로 계좌인증 서비스가 불가합니다.");
				else if(result.resultCode == "OVER") $("#authAccountMsgFail").html("해당 은행의 전산 장애로 계좌인증 서비스가 불가합니다.");
				else if(result.resultCode == "D888") $("#authAccountMsgFail").html("해당 은행의 시스템 작업으로 계좌인증 서비스가 불가합니다. 해당 은행의 서비스 이용시간을 확인하시기 바랍니다.");
				else if(result.resultCode == "D999") $("#authAccountMsgFail").html("해당 은행의 시스템 작업으로 계좌인증 서비스가 불가합니다. 해당 은행의 서비스 이용시간을 확인하시기 바랍니다.");
				else if(result.resultCode == "C001") $("#authAccountMsgFail").html("해당 은행의 전산 장애로 계좌인증 서비스가 불가합니다.");
				else if(result.resultCode == "C002") $("#authAccountMsgFail").html("해당 은행의 전산 장애로 계좌인증 서비스가 불가합니다.");
				else if(result.resultCode == "C003") $("#authAccountMsgFail").html("해당 은행의 전산 장애로 계좌인증 서비스가 불가합니다.");
				else $("#authAccountMsgFail").html("예금주와 구매자명이 일치하지 않습니다.");
...
var fncGoSearchPop = function(){
	window.open("/product-pop.do", "제품정보검색", "scrollbars=yes,width=940,height=800");
};

var fncGoBranchPop = function(){
	window.open("/branch-pop.do", "매장정보검색", "scrollbars=yes,width=820,height=800");
};
