$(function () {
    ...
    $('.sub_header').clone().attr('aria-hidden','true').addClass('fixed').removeClass('nb').appendTo('body');
    var s = 1;
    $(window).on('scroll',function(){
        ...
        if ($(window).scrollTop() >= $('header').height()){
            ...
            if (s != 1 ) {
                $('.sub_header.fixed').css('top',0).removeClass('nb');
    ...
    $('.btn_cal_confirm').click(function(){
        ...
        if ($layerBtn.hasClass('btn_calendar')) {
            var $layerCon = $(this).parents('.layer_con'),
                ...
                date =  year + '/' + month + '/' + $layerCon.find('td.start a.dateNum').text();
            ...
            for (var i=0;i<=1;i++) {
                ...
                $('.' + rr[i] + '_hour').val($calDate.find('.sel_hour').val());
                $box.find('.hour').html($calDate.find('.sel_hour').val() + 'h');

                $('.' + rr[i] + '_minute').val($calDate.find('.sel_minute').val());
                $box.find('.minute').html($calDate.find('.sel_minute').val() + 'm');
