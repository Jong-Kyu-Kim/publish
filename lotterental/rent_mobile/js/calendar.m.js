...
LCalendar.prototype._initEvent = function(selector,date) {
	...
	this.$calendar.find('a.dateNum').click(function(e){
		...
		if (objThis.start == true) {
			...
			$(this).parents('.calendar_b').siblings('.rent').find('.date').html(objThis.selectedRentYear + '/' + objThis.selectedRentMonth + '/' + objThis.selectedRentDate);
...
$(function(){
    var mCalendar1 = new LCalendar('.calendar_b');
   	...
   	// $('.calendar_b').find('a.dateNum:first').trigger('click');
});